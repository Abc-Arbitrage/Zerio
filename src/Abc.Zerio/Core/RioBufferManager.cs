...
{
    public unsafe class RioBufferManager : IDisposable
    {
        ...
        private RioBufferManager(int bufferCount, int bufferLength)
        {
            ...
            _bufferDescriptors = Kernel32.VirtualAlloc(IntPtr.Zero, (uint)(sizeof(RIO_BUF) * bufferCount), Kernel32.Consts.MEM_COMMIT | Kernel32.Consts.MEM_RESERVE, Kernel32.Consts.PAGE_READWRITE);
        ...
        {
            buffer = null;
            if (!_freeBufferIds.TryPop(out var segmentId))
                return false;
